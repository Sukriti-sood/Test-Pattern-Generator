...
def parse_equation(equation):
  ...
  return {
    'gate': gate,
    'inputs': [input_node.strip() for input_node in inputs]
  }
...
def dfs(circuit, current_node, value, vis, node_values):
  ...
  vis[current_node] = 1
  if (current_node == 'A' or current_node == 'B' or current_node == 'C'
      or current_node == 'D'):
    return
  ...
  if (gate == '&'):
    ...
    cnt = 0
    # ek.. pr bhi 1 de denge to
    for nodes in circuit[current_node]['inputs']:
      if (cnt == 0 and value == 1):
        dfs(circuit, nodes, 1, vis, node_values)
      else:
        dfs(circuit, nodes, 0, vis, node_values)
      cnt += 1
    return
  for nodes in circuit[current_node]['inputs']:
    dfs(circuit, nodes, val, vis, node_values)
...
if __name__ == "__main__":
  ...
  with open("output.txt", "w") as output_file:
    for test_vector, expected_output in test_vectors:
      output_file.write(
        f"[A, B, C, D]: {list(test_vector)}, Z= {expected_output}\n")
